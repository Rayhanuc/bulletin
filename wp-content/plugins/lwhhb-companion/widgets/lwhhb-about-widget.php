<?php
// Adds widget: lwhhbAbout
class Lwhhbabout_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'lwhhbabout_widget',
			esc_html__( 'Bulletin About', 'lwhhb' ),
			array( 'description' => esc_html__( 'About This Site Widget', 'lwhhb' ), ) // Args
		);
	...
	private $widget_fields = array(
		array(
			'label' => 'About Image',
			'id' => 'lwhhb_about_image',
			'type' => 'media',
		),
		array(
			'label' => 'Description',
			'id' => 'lwhhb_about_description',
			'type' => 'textarea',
		),
		array(
			'label' => 'Signature',
	...
	public function widget( $args, $instance ) {
		...
		if($instance['lwhhb_about_image']){
			echo wp_get_attachment_image($instance['lwhhb_about_image'],'medium');
		}
		echo '<p class="mt-3"><em>'.esc_html($instance['lwhhb_about_description']).'</em></p>';

		if($instance['lwhhb_signature']){
			echo wp_get_attachment_image($instance['lwhhb_signature'],'medium');
		}
...
function register_lwhhbabout_widget() {
	register_widget( 'Lwhhbabout_Widget' );
}
add_action( 'widgets_init', 'register_lwhhbabout_widget' );